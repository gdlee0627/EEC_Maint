...
public class CheckAIStatus : BaseNetLogic
{
    [ExportMethod]
    public void Check()
    {
    ...
    private void checkLifeCycle(LongRunningTask task)
    {
        ...
        myStore.Query("SELECT * FROM AI", out header, out resultSet);
        ...
        // doc = new HtmlWeb().Load("https://www.rockwellautomation.com.cn/products/details.1756-L85EP.html");
            ...
            myStore.Query("UPDATE AI SET Status = " + "\""+ node2.InnerText.Trim()+ "\"" + " WHERE ID = " + resultSet[i, 0] , out headers, out resultSets);
        }
