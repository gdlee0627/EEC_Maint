...
public class CheckDIStatus : BaseNetLogic
{
[ExportMethod]
    public void Check()
    {
    ...
    private void checkLifeCycle(LongRunningTask task)
    {
        ...
        myStore.Query("SELECT * FROM DI", out header, out resultSet);
        ...
        // doc = new HtmlWeb().Load("https://www.rockwellautomation.com.cn/products/details.1756-L85EP.html");
            ...
            myStore.Query("UPDATE DI SET Status = " + "\""+ node2.InnerText.Trim()+ "\"" + " WHERE ID = " + resultSet[i, 0] , out headers, out resultSets);
        }
